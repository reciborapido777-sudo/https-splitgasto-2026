...
/* ── 1. Cargar traducciones existentes ── */
...
files.forEach(file => {
  ...
  $(selectors).each(function(i){
    ...
    if (/^[—\d\s\.,€$¥£%\(\)\/\+\-\*]+$/.test(txt)) return;   // números/moneda
    if (/^[^\p{L}]*$/u.test(txt)) return;                       // solo símbolos/emojis
...
/* ── 4. Guardar i18n.js preservando funciones ── */
